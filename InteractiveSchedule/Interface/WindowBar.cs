...
{
	/// <summary>
	/// The header bar of a <see cref="WindowComponent"/>.
	/// Generated by <see cref="WindowPage"/> and paired when one is instantiated.
	/// </summary>
	public class WindowBar : WindowComponent
	{
		/// <summary>
		/// Window title to be drawn in the header bar.
		/// </summary>
		public string DisplayName;
		public override bool IsSelected =>_childMenu == null || (_childMenu is WindowComponent window && window.IsSelected);
		/// <summary>
		...
		/// <summary>
		/// Whether the content is temporarily hidden exclusive of <see cref="IsMinimised"/>.
		/// </summary>
		public bool IsContentHidden;
		/// <summary>
		/// Whether the child <see cref="WindowPage"/> should be visible and interactible.
		/// </summary>
		...
		/// <summary>
		/// Source area for the header bar window icon also used in <see cref="Taskbar.Icons"/>.
		/// </summary>
		...
		private static readonly Point Dimensions = new Point(128, BarButtonDimensions.Y * ButtonScale);
		...
		public WindowBar(WindowComponent childMenu, Point position) : base()
		{
		...
		public void RememberWindowSize()
		{
			...
			AreaBeforeFullscreen = new Rectangle(
				xPositionOnScreen,
				yPositionOnScreen,
		...
		public void SetForChildMenu(IClickableMenu childMenu)
		{
			...
			width = _isFullscreen ? Game1.viewport.Width - Desktop.Taskbar.width : AreaBeforeFullscreen.Width;
		...
		public override void leftClickHeld(int x, int y)
		{
			...
			++_leftClickHeldTimer;
			if (_leftClickHeldTimer > 10f * LeftClickHeldDelayScale && (IsContentHidden || this.isWithinBounds(x, y)))
			{
				// Drag window around screenspace, but keep window within viewport bounds
				Point delta = new Point(Game1.getMouseX() - Game1.getOldMouseX(), Game1.getMouseY() - Game1.getOldMouseY());
				xPositionOnScreen = Math.Max(Desktop.Taskbar.width - (1 * MenuScale), Math.Min(Game1.viewport.Width - 64, xPositionOnScreen + delta.X));
				yPositionOnScreen = Math.Max(0, Math.Min(Game1.viewport.Height - height, yPositionOnScreen + delta.Y));
		...
		public static void DrawWindowBarContainer(SpriteBatch b, int x, int y, int w, int h, Color colour, bool greyed, bool simpleStyle = false, bool drawShadow = false)
		{
			...
			if (drawShadow)
			{
				b.Draw(
					...
					color: CustomMenu.ShadowColour * CustomMenu.ShadowOpacity);
			}
			...
			if (greyed)
			{
				...
				b.Draw(
					...
					color: Color.Black * CustomMenu.ShadowOpacity);
			}
